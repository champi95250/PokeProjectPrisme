...
class LocationWindow
  ...
  def initialize(name)
    ...
    if @extended_route
      textos = [
        [name, 73, 8, 0, Color.new(255,255,255), Color.new(115,115,115)]
      ]
    else
      textos = [
        [name, 59, 8, 0, Color.new(255,255,255), Color.new(115,115,115)]
      ]
    end
    #Draws the route numbers as Graphics...
    if @route_number
      if @route_number.to_i >= 10
        pbDrawRouteNumber(@route_number, overlay, 24 - 16, 8 + 2)
      else
        pbDrawRouteNumber(@route_number, overlay, 24 - 6, 8 + 2)
      end
    end

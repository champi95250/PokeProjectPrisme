...
------------------------------------------------------
  ...
  "deep_blue"                   | arg 8 |         => Tone of the custom graphics, there's 12 colors by default, you can add more
  )
...
class TournamentSelection
  ...
  TRAINER_FACE = false #if true trainer name text will be replaced by their battle sprite

  attr_reader :oppo_team

  def initialize(poke_max, trainer_type_0,trainer_name_0,trainer_ver_0 = 0, show_items = true, battle_start = false, param = nil, input_tone = "default")
    ...
    tones_hash = { #adds tone on the custom graphics (default is red and orange like fire)
      "default" => {r:0,v:0,b:0,a:0},
      "red" => {r:75,v:-100,b:-100,a:255},
      "orange" => {r:80,v:10,b:-80,a:200},
      "yellow" => {r:100,v:80,b:-50,a:255},
      "lime" => {r:40,v:90,b:-110,a:255},
      "green" => {r:-100,v:75,b:-80,a:255},
      "cyan" => {r:-100,v:60,b:60,a:255},
      "blue" => {r:-50,v:40,b:160,a:255},
      "deep_blue" => {r:-100,v:-70,b:100,a:255},
      "purple" => {r:-50,v:-150,b:50,a:255},
      "magenta" => {r:50,v:-100,b:50,a:255},
      "grey" => {r:0,v:0,b:0,a:255}
      #add your custom color tones here
    }
    tones_hash.each do |key, value|
      @tone = value if key == input_tone
    end
  ...
  def draw_img
    ...
    @sprites["textsprites6"] = Sprite.new(@viewport)
    @sprites["textsprites6"].bitmap = Bitmap.new(Graphics.width, Graphics.height)
    @sprites["textsprites6"].z = 6
    ...
    @sprites["ball_bar"].tone = Tone.new(@tone[:r],@tone[:v],@tone[:b],@tone[:a])
    ...
    draw_animation("particles", "Graphics/UI/Tournament Selection/particles", 7)
    draw_balls
    textpos4 = [["Please choose #{@poke_max} POKÉMON.", 148, 11, :left, Color.new(252, 252, 252),nil]]
...
=end
  ...
  def draw_balls
    ...
    @poke_max.times do |i|
      ...
      @sprites["ball_slot#{i}"].z = 3
      @sprites["ball_slot#{i}"].tone = Tone.new(@tone[:r],@tone[:v],@tone[:b],@tone[:a])
      #-----------------------------------------------
      ...
      @sprites["ball_opp_slot#{i}"].z = 3
      @sprites["ball_opp_slot#{i}"].tone = Tone.new(@tone[:r],@tone[:v],@tone[:b],@tone[:a])
      x_ball -= 17
      x_ball_opp += 17
    end
  end
  ...
  def draw_party(side)
    ...
    if side == "player"
      ...
      x_gender = 36
      @poke_max
      textpos6.push([trainer.name, 202 - (@poke_max * 17), 40, 1, base, nil]) if !TRAINER_FACE
    elsif side == "opponent"
      ...
      textpos1.push([trainer.name, 312 +(@poke_max * 17), 40, 0, base, nil]) if !TRAINER_FACE
    end
    trainer.party.each_with_index do |pkmn, i|
        ...
        @sprites["poke_bar_#{side}_#{i}"].tone = Tone.new(@tone[:r],@tone[:v],@tone[:b],@tone[:a])
        if side == "player"  #sel bars
          ...
          @sprites["poke_sel_bar_#{side}_#{i}"].tone = Tone.new(@tone[:r],@tone[:v],@tone[:b],@tone[:a])
        end
        ...
        if side == "player"
          ...
          @sprites["nums#{i}"].tone = Tone.new(@tone[:r],@tone[:v],@tone[:b],@tone[:a])
        end
    ...
    if side == "player"
      ...
      @sprites["trainer_#{side}"].src_rect.set(0,0,@sprites["trainer_#{side}"].bitmap.width,56)
      @sprites["trainer_#{side}"].mirror = true
      @sprites["trainer_#{side}"].x = -(@sprites["trainer_#{side}"].bitmap.width / 4)
      @sprites["trainer_#{side}"].y = 0
      @sprites["trainer_#{side}"].z = 2
      @sprites["trainer_#{side}"].visible = false if !TRAINER_FACE
    elsif side == "opponent"
      ...
      unless @sprites["trainer_#{side}"].bitmap == nil
        @sprites["trainer_#{side}"].src_rect.set(0,0,@sprites["trainer_#{side}"].bitmap.width,56)
        @sprites["trainer_#{side}"].x = Graphics.width - @sprites["trainer_#{side}"].bitmap.width
        @sprites["trainer_#{side}"].y = 0
        @sprites["trainer_#{side}"].z = 2
        @sprites["trainer_#{side}"].visible = false if !TRAINER_FACE
      end
  ...
  def draw_animation(name, path, z = 0)
    Dir.glob(File.join(path, "*.{png,jpg,jpeg,bmp}")).each_with_index do |file, i|
      ...
      @sprites["#{name}_#{i}"].tone = Tone.new(@tone[:r],@tone[:v],@tone[:b],@tone[:a])
    end
  end
  ...
  def draw_confirm_message(sel)
    ...
    pbDrawTextPositions(textsprite,
                        [[_INTL("Are you happy with these choices?"), 80, y + 26, :left, Color.new(76, 76, 76), shadow]])
      ...
      if Input.trigger?(Input::USE)
        ...
        when 0
          if team_data["slot#{sel}"]["choose_num"] != nil
            ...
            if (team_data["slot#{sel}"]["choose_num"] == @poke_max && $player.party.length >= @poke_max) || (team_data["slot#{sel}"]["choose_num"] == $player.party.length && $player.party.length < @poke_max)
              pbPlayDecisionSE

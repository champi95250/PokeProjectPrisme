...
def choose_party(party,param,poke_max)
  if param == "EXEMPLE"      #you can copy/paste this an only modify numbers
    ...
    return new_team
      ...
      fripichat = party.find { |pkmn| pkmn.species == :FRIPICHAT }

      if !fripichat
        raise "Pierrot doit obligatoirement posséder un FRIPICHAT."
      end

      available_pokemon = party.reject { |pkmn| pkmn == fripichat }
      selected_pokemon = available_pokemon.sample(poke_max - 1)

      new_team = selected_pokemon + [fripichat]

      new_team.each { |pkmn| pkmn.calc_stats }
      return new_team
  elsif param == "ROMAIN"
      fripichat = party.find { |pkmn| pkmn.species == :QUILAVA }

      if !fripichat
        raise "Pierrot doit obligatoirement posséder un QUILAVA."
      end
